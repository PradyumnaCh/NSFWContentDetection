...
class LSTMAttention(nn.Module):
    def __init__(self, opts, vectors):
        ...
        self.embed_layer = nn.Embedding(
            opts.vocab_size, opts.embed_dim, padding_idx=opts.pad_idx
        )
        ...
        self.lstm = nn.LSTM(
            ...
            num_layers=opts.num_rnn_layers,
            batch_first=True,
        ...
        self.fc_layer = nn.Linear(
            opts.hidden_dim * opts.num_rnn_layers, opts.num_labels
        )
    ...
    def forward(self, input_ids, seq_len, label_ids=None):
        ...
        packed_reps = pack_padded_sequence(
            token_reps, batch_first=True, lengths=seq_len, enforce_sorted=False
        )
        ...
        if self.bidirectional:
            last_hidden = torch.cat(
                [last_hidden[i, :, :] for i in range(last_hidden.shape[0])], dim=-1
            )
